...
def sphconv(inputs, filters, use_bias=True, n_filter_params=0,order=0,
            *args, **kwargs):
    shapei = tfnp.shape(inputs)
    spectral_input = True if len(shapei) == 5 else False
    nchan = shapei[-1]
    shapei =tfnp.shape( kwargs.get('harmonics_or_legendre'))
    order = shapei[1]
    n=order
   ...
   # if spectral_input:
    ...
    with tf.variable_scope(None, default_name='sphconv'):
        ...
        #                           trainable=True,
        #                           initializer=tf.truncated_normal([nchan, n , filters],
        #                                                           stddev=std),
        #                           ) # regularizer=tf.contrib.layers.l2_regularizer(.9)
        ...
        # print(inputs.shape)
        conv = sphericalcof.sph_conv_batch(inputs, ker, *args, **kwargs)
...
def block(params, fun, is_training=None, *args, **kwargs):
    ...
    order = params.order
    if fun == sphconv:
     curr = fun(*args, **kwargs, use_bias=use_bias,order=order)
    else:
    ...
    if params.batch_norm:
        ...
        #                                      training=is_training,
        #                                     )
       # curr = tf.contrib.layers.instance_norm( curr )
