...
def sph_harm_all(n, order=7, as_tfvar=False, real=False):
...
def sph_harm_transform(f, mode='DH', order=7, harmonics=None):
...
def sph_harm_inverse(c,res=64, harmonics=None):
...
def sph_harm_transform_batch(f,  method=None, order=None, *args, **kwargs):
    return sph_harm_transform_batch_naive(f, order, *args, **kwargs)


def sph_harm_inverse_batch(c, n, method=None, *args, **kwargs):
    return sph_harm_inverse_batch_naive(c, n, *args, **kwargs)


def sph_harm_transform_batch_naive(f, order=7, harmonics=None, m0_only=False):
    ...
    assert shapef[2] == l
    if harmonics is None:
        harmonics = sph_harm_to_shtools(sph_harm_all(l,order=order))
...
def sph_harm_inverse_batch_naive(c,res=64, harmonics=None):
    ...
    # assert shapec[3] == l
    if harmonics is None:
        harmonics = sph_harm_to_shtools(sph_harm_all(l, order = shapec[3]))
    shapeh = tfnp.shape(harmonics)
    assert shapeh[1:] == (shapec[3], shapec[3], l, l)
    assert shapeh[0] in [1, 2]
...
def sph_conv_batch(f, g,
   ...
   # if spectral_input:
    ...
    if not spectral_input:
        cf = sph_harm_transform_batch(f,  method, order , harmonics_or_legendre,   m0_only=False)
    else:
        cf = f
    if not spectral_filter:
        cg = sph_harm_transform_batch(g, method, order , harmonics_or_legendre,  m0_only=True)
    else:
        cg = g
...
#    print(shapecg)
    ...
    na = np.newaxis
    # per degree factor
    factor = (2*np.pi*np.sqrt(4*np.pi / (2*np.arange(shapecg[2])+1)))[na, na, :, na, na, na]
    ...
    if spectral_pool > 0:
        cfg = cfg[:, :, :n//(2**(spectral_pool)), :n//(2**(spectral_pool)), ...]
